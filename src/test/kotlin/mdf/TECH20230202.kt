    ...
    private fun p3(lines: List<String>): String {
        ...
        while (t < top.size || b < bottom.size) {
            ...
            points.add(when (diff.sign) {
                -1.0 -> Point(top[t].x, top[t].y - Shape.interpolate(top[t].x, bottom[b - 1], bottom[b])).also { t++ }
                1.0 -> Point(bottom[b].x, Shape.interpolate(bottom[b].x, top[t - 1], top[t]) - bottom[b].y).also { b++ }
